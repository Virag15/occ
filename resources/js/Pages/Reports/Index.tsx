...
import { BarChart3, Crown, Hourglass, Tags, Truck } from 'lucide-react';
...
// Recharts brand palette — distinct hues for the pie. Reuses dashboard colour vocabulary.
    ...
    return (
        <AdminLayout breadcrumbs={[{ label: 'Reports' }]}>
            ...
            <div className="mb-4 flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">· {month_label} unless noted</span>
            </div>
            ...
            <div className="mb-5 grid gap-5 lg:grid-cols-2">
                <Card>
                    ...
                    <CardContent className="p-4 pt-2">
                        {brand_mix.length === 0 ? (
                            <p className="py-6 text-center text-sm text-muted-foreground">No tagged brand sales this month.</p>
                        ) : (
                ...
                <Card>
                    <CardHeader className="p-4 pb-2">
                        ...
                        <p className="text-[10px] text-muted-foreground">Last 90 days · sortable signal for transporter performance</p>
                    </CardHeader>
                    <CardContent className="p-0">
                        {dispatch_sla.length === 0 ? (
                            <p className="px-4 py-6 text-center text-sm text-muted-foreground">No dispatches in the last 90 days.</p>
                        ) : (
            ...
            {/* Bottom row — top customers (full) + aging (expandable buckets) */}
            <div className="grid gap-5 lg:grid-cols-2">
                <Card>
                    <CardHeader className="p-4 pb-2">
                        <CardTitle className="flex items-center gap-1.5 text-sm font-medium">
                            <Crown className="h-4 w-4 text-amber-500" />
                            Top customers · {month_label}
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="p-0">
                        {top_customers.length === 0 ? (
                            <p className="px-4 py-6 text-center text-sm text-muted-foreground">No bookings this month yet.</p>
                        ) : (
